...
import { NeuralNetworkButton } from "./button";
...
export default function Home() {
  ...
  try {
    ...
    const res = await axios.get("http://127.0.0.1:5000/bot-chat", {
      params: { human_input: humanInput, auth_token: token },
    });
    ...
    const botMsg = {
      ...
      createdAt: new Date(),
      userId: 0,
      content: res.data, // depending on backend format
    };
  ...
  const initialMessages = async () => {
    const token = localStorage.getItem("auth_token");
    if (!token) return alert("No auth token found");
    const messages = await getUserMessages(token, page);
  ...
  const handleScroll = async () => {
    ...
    if (box.scrollTop === 0) {
      ...
      const token = localStorage.getItem("auth_token");
      if (!token) return alert("No auth token found");
      const older = await getUserMessages(token, nextPage);
